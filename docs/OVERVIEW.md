...
- **U-Net candidate detection** (`services/unet/`) — LBNL inference pipeline to
  expand the candidate universe beyond the current 6 states (GPU host required).
...
**Latent / structural**
- **Two U-Net trees**: `UNET/` (research + EDX dataset notes) vs
  `services/unet/` (inference module). Different lifecycles, intentionally not
  merged — `services/unet/` is authoritative for inference. Cross-linked in
  `CLAUDE.md`.
