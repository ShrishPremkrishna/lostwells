...
## 5. Next steps to reach the true vision

**Tier 1 — make the impact score honest and complete**
1. Implement **drinking-water proximity** (EPA SDWIS / Community Water System
   service-area point-in-polygon) and **hospitals** (HIFLD `hifld-next` GitHub
   mirror) so the 45% human-exposure pillar is fully computed, not 82% of it.
2. Replace tract-population with **true 1-mile population** (areal interpolation
   over block groups, or a dasymetric/gridded surface).
3. Swap the EJ proxy for **PEDP EJScreen** (GitHub mirror) and disclose provenance.
4. Pull **depth/type/status** from state DBs (PA DEP, OK OCC, CalGEM) for the hero
   wells and any matchable candidates, so methane and plug cost actually
   differentiate instead of being constants.

**Tier 2 — make the detection real and national**
5. Run `services/unet/infer.py` on a GPU host; **validate on Kern County** against
   LBNL's published detections; then a sparse stratified pass across ~15–25 legacy
   oil/gas states; merge new detections into `candidates.*`.
6. Reconcile the 1,303-vs-1,301 discrepancy.

**Tier 3 — make the hero moment and live AI bulletproof**
7. Geocode the hero wells to authoritative addresses; fix citations to the real
   source documents; bundle **georeferenced 1950s HTMC quads** (rio-tiler /
   `gdal2tiles`) so the topo-dissolve shows the actual period map.
8. Add a **live investigation API route** (TS `anthropic` SDK + web search) so the
   in-browser swarm panel streams a *real* run, not a replay.
9. Add **per-claim source verification / confidence scoring** to dossiers, and a
   cache-refresh path.

**Tier 4 — product polish toward the spec**
10. Add the **adjustable-weight slider** panel (scorer already supports it).
11. Browser-based visual + performance QA of the 117k-point render; mobile,
    accessibility, empty/error/loading states.
12. Deploy (Vercel) with the committed datastore; record a fallback demo video.

---

## 6. One-line summary

A real, honest, working investigative product — the backbone data, the transparent
ranking, the live Claude web-search swarm, and the award-grade UI are all genuine —
**held back from its full vision by incomplete exposure metrics (no drinking-water/
hospitals), undocumented-well metric uniformity, estimated hero coordinates, an
unexecuted U-Net, and the lack of an in-browser visual check.** None of these are
hidden in the product; closing them (Tier 1–2 first) is what turns a strong demo
into a defensible decision tool.
